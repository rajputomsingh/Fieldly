...
export function useMarketplace(
  initialFilters: FeedFilters = { sortBy: "hotnessScore" },
  ...
  const fetchListings = useCallback(
    async (pageNum: number, reset: boolean = false) => {
      try {
        ...
        if (debouncedFilters.landType)
          params.append("landType", debouncedFilters.landType);
        if (debouncedFilters.state)
          params.append("state", debouncedFilters.state);
        if (debouncedFilters.district)
          params.append("district", debouncedFilters.district);
        if (debouncedFilters.minSize)
          params.append("minSize", debouncedFilters.minSize.toString());
        if (debouncedFilters.maxSize)
          params.append("maxSize", debouncedFilters.maxSize.toString());
        if (debouncedFilters.irrigation) params.append("irrigation", "true");
        if (debouncedFilters.verifiedOnly)
