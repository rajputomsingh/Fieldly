...
function getAuctionData(data: AuctionData | AuctionDTO | null): AuctionData {
  ...
  if ("listing" in data) {
    ...
    return {
      ...
      basePrice: Number(a.listing?.basePrice || 0),
      bidIncrement: Number(a.listing?.bidIncrement || 1000),
      ...
      highestBid: Number(a.listing?.highestBid || 0),
      winningBid: a.listing?.winningBid,
      _count: { bids: a.stats?.totalBids ?? 0 },
      bids: a.listing?.bids || [],
    };
  }
  // Ensure plain object also has numbers
  const plainData = data as AuctionData;
  return {
    ...plainData,
    basePrice: Number(plainData.basePrice || 0),
    bidIncrement: Number(plainData.bidIncrement || 1000),
    highestBid: Number(plainData.highestBid || 0),
  };
}
...
export function AuctionRoomClient({
  ...
  const { auction, bids, loading, placeBid, timeRemaining, isLive, error } =
  ...
  // FIX: Ensure numbers with Number()
  const highestBid = Number(
    currentBids[0]?.amount || currentAuction?.basePrice || 0,
  );
  const bidIncrement = Number(currentAuction?.bidIncrement || 1000);
  const minNextBid = highestBid + bidIncrement;
          ...
          <Card className="p-6">
            ...
            <dl className="space-y-3">
              <div className="flex justify-between">
                ...
                <dd className="font-medium">{formatCurrency(highestBid)}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-muted-foreground">Bid Increment</dt>
                <dd className="font-medium">{formatCurrency(bidIncrement)}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-muted-foreground">Ends At</dt>
                <dd className="font-medium">
                  {new Date(currentAuction.endDate)
                    .toISOString()
                    .replace("T", " ")
                    .substring(0, 19)}
                </dd>
