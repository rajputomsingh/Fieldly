...
class MarketplaceService {
  ...
  async getListing(
    ...
    return this.measure("getListing", async () => {
      ...
      const transformed = transformers.transformListingDetail(
  ...
  async getAuction(listingId: string): Promise<AuctionDTO | null> {
    return this.measure("getAuction", async () => {
      ...
      const transformed = transformers.transformAuction(
        auctionData as Record<string, unknown>,
      );
    ...
    return this.measure("placeBid", async () => {
      ...
      const result = await prisma.$transaction(
        async (tx) => {
          ...
          if (
            timeLeft < MARKETPLACE_CONSTANTS.BID.AUTO_EXTEND_WINDOW &&
      ...
      await auctionEvents.emitNewBid(listingId, {
        id: result.bid.id,
        amount: result.bid.amount?.toNumber() ?? 0,
      ...
      return {
        ...
        timing: {
          durationMs: Math.round(performance.now() - performance.now()),
        }, // Will be captured by measure()
      };
    });
  }
  ...
  async getRecommendations(userId: string) {
    return this.measure("getRecommendations", () =>
